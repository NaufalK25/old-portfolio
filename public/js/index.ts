...
const getRepoStack = (repoName: string): string[] => {
    return ['typescript', 'ejs', 'css', 'shell'];
}

// My Projects Section
const myProjectsSection = <HTMLDivElement>document.querySelector('section#my-project-list');

fetch('https://api.github.com/users/naufalk25/repos?sort=updated')
        ...
        repos.forEach((repo: any): void => {
            if (repo.full_name !== 'NaufalK25/NaufalK25') {
                ...
                repo.topics.forEach((topic: string): void => {
                    ...
                    repoTopicsList.appendChild(repoTopic);
                });

                repoTopics.appendChild(repoTopicsSummary);
                repoTopics.appendChild(repoTopicsList);
                ...
                const githubSVG = new DOMParser().parseFromString(
                    `
                ...
                repoLink.appendChild(repoLink.ownerDocument.importNode(githubSVG.documentElement, true));
